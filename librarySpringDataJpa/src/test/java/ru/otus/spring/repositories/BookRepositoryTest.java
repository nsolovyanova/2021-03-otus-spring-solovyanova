...
@DisplayName("Тест репозитория на основе Jpa для работы со книгами BookRepository")
@DataJpaTest
class BookRepositoryTest {
    ...
    void shouldReturnBookByName() {
        val actualBook = repository.findByName(FIRST_BOOK_NAME);
        val expectedBook = Optional.of(em.find(Book.class, FIRST_BOOK_ID));
        assertThat(actualBook).usingFieldByFieldValueComparator().isEqualTo(expectedBook);
    }
    ...
    void shouldReturnAllBooksByAuthor() {
        ...
        val books = repository.getAllBookByAuthor(author);
    ...
    void shouldReturnAllBooksByGenre() {
        ...
        val books = repository.getAllBookByGenre(genre);
